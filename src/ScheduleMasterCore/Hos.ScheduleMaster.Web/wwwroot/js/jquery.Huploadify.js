(function ($) {
    $.fn.Huploadify = function (opts) {
        var itemTemp = '<div id="${fileID}" class="uploadify-queue-item" style="margin:10px 0"><div class="uploadify-progress progress progress-striped active" style="margin-bottom:10px"><div class="uploadify-progress-bar progress-bar progress-bar-success" role="progressbar" aria-valuemin="0" aria-valuemax="100"></div></div><span class="up_filename" style="margin-right:8px">${fileName}</span><span class="uploadbtn label label-info"  style="margin-right:8px;cursor: pointer;">上传</span><span class="delfilebtn label label-darkorange" style="cursor: pointer;">删除</span></div>';
        var defaults = {
            ...
            removeTimeout: 1500,//上传完成后进度条的消失时间
        ...
        this.each(function () {
            ...
            inputStr += '"/>';
            inputStr += '<a id="file_upload_' + instanceNumber + '-button" href="javascript:void(0)" class="uploadify-button btn btn-info">';
            ...
            var fileObj = {
                ...
                onProgress: function (file, loaded, total) {
                    ...
                    if (percent == '100.00%') setTimeout(function () { eleProgress.fadeOut() }, option.removeTimeout);
                },		
                        ...
                        xhr.onreadystatechange = function (e) {
                            if (xhr.readyState == 4) {
                                if (xhr.status == 200) {
                                    ...
                                    //在指定的间隔时间后删掉进度条
                                    //setTimeout(function(){
                                    //                         _this.find('#fileupload_' + instanceNumber + '_' + file.index + ' .uploadify-progress').fadeOut();
