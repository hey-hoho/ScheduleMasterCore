...
namespace Hos.ScheduleMaster.QuartzHost.Common
{
    ...
    public class RootJob : IJob
    {
                    ...
                    try
                    {
                        ...
                        if (job.JobDataMap["instance"] is TaskBase instance)
                        {
                            ...
                            try
                            {
                                ...
                                stopwatch.Stop();
                                UpdateRunTrace(traceId, stopwatch.Elapsed.TotalSeconds, ScheduleRunResult.Success);
                            ...
                            {
                                stopwatch.Stop();
                                UpdateRunTrace(traceId, stopwatch.Elapsed.TotalSeconds, ScheduleRunResult.Conflict);
                                throw conflict;
                            }
                            catch (Exception e)
                            {
                                stopwatch.Stop();
                                UpdateRunTrace(traceId, stopwatch.Elapsed.TotalSeconds, ScheduleRunResult.Failed);
        ...
        private void UpdateRunTrace(Guid traceId, double elapsed, ScheduleRunResult result)
        {
            ...
            _db.Database.ExecuteSqlRaw($"update scheduletraces set result={(int)result},elapsedtime={elapsed} where traceid='{traceId}'");
        }
    }
}